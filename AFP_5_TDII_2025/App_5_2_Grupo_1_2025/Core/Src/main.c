...
delayInit(&timer_inicio, retardo);

bool_t estado_led = false; // Almacena en que fase de la secuencia se encuentra el led. true-> Encendido, false-> Apagado.

//Variables para los cambios de secuencia
int8_t i=0;
bool_t toggle_Boton=false;
bool_t switch_secuencia= true;
...
{
	toggle_Boton= !toggle_Boton;

if (toggle_Boton == true)

		{
			 //Cambio a secuencia  A
	             switch_secuencia= false;
		}
		else
		{
			//Cambio a suciencia B
			  switch_secuencia= true;
		}
	}
